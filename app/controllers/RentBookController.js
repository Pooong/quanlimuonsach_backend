...
class RentController {
	async addToRent(req, res, next) {
		try {
			const { maDocGia, maSach, ngayMuon, ngayTra, soLuong, thanhTien } = req.body;
			...
			if (soLuong > existingSach.SoQuyen) return res.json({ error: "Số lượng hàng không đủ" });
		...
		try {
			...
			if (searchQuery) {
				TheoDoiMuonSach.find(query)
					...
					.then((theoDoiMuonSachs) => {
						const filteredResults = theoDoiMuonSachs.filter((item) => item.MaDocGia !== null);
